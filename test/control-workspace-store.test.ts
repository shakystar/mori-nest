 ...
 * 그 이상 만들지 않는다.
 *
 * 목록 조회·포크 판정 테스트는 `openedAt` 순서/겹침을 검증해야 하는데 `insertMintedWorkspace`는
 * `new Date()`로 `openedAt`을 정하므로(주입 지점이 없다), 같은 밀리초에 걸리면 순서·경계
 * 단언이 들쭉날쭉해진다. `vi.useFakeTimers()` + `vi.setSystemTime`으로 각 개시 사이의 시각을
 * 고정해 결정적으로 만든다.
 *
 * `WorkspaceStore.openWorkspace`는 라우트가 안 쓰게 된 자기 트랜잭션 재시도 경로라 걷혔다
 * (mori-nest #140, PR #139 교차 지적 회수) — 이 파일의 개시 픽스처는 이제 {@link insertWorkspace}
 * 하나로 `mintWorkspaceId` + `insertMintedWorkspace`를 라우트와 같은 순서로 조합한다.
 */
...
/** 라우트가 하는 mint + insert 조합을 시험에서 재현한다 — `WorkspaceStore.openWorkspace`가
 * 걷힌 뒤(mori-nest #140) 개시 픽스처를 만드는 유일한 경로다. */
async function insertWorkspace(
  ...
  const workspaceId = store.mintWorkspaceId()
  await database.withTransaction(() => {
    store.insertMintedWorkspace(workspaceId, subject, request)
  })
  return { workspaceId }
}

/** 개시 사이에 `vi.setSystemTime`으로 시각을 옮기며 여러 작업공간을 연다 — 파일 상단 doc
 * "목록 조회 테스트는 openedAt 순서를 검증해야 하는데" 참고. {@link insertWorkspace}의
...
async function openSequenced(
  ...
  try {
    for (const timeMs of timesMs) {
      ...
      const { workspaceId } = await insertWorkspace(database, store, subject, { logs: ['log-a'] })
...
describe('WorkspaceStore 개시(insertMintedWorkspace) + getWorkspace', () => {
  it('개시 직후 단건 조회는 active이고 lastHeartbeatAt === openedAt, logs가 요청 그대로이며 replicaId 미신고 시 그 필드가 없다', async () => {
    ...
    const store = await openWorkspaceStore(database)

    const { workspaceId } = await insertWorkspace(database, store, 'alice', { logs: ['log-a', 'log-b'] })
  ...
  it('lastHeartbeatAt + gracePeriod가 지난 now로 조회하면 abandoned이고, endedAt은 조회 시각과 무관하게 같다', async () => {
    ...
    const { workspaceId } = await insertWorkspace(database, store, 'alice', { logs: ['log-a'] })
  ...
  it('supersedes로 같은 주체의 이전 작업공간을 이으면 개시가 성공하고, 이전 작업공간의 조회에 supersededBy가 붙는다', async () => {
    ...
    const { workspaceId: first } = await insertWorkspace(database, store, 'alice', { logs: ['log-a'] })
    const { workspaceId: second } = await insertWorkspace(database, store, 'alice', {
      logs: ['log-a'],
      supersedes: first,
    })
  ...
  it('supersedes가 다른 주체의 작업공간이면 개시가 실패하고 기록이 남지 않는다', async () => {
    // 첫 삽입은 alice의 기존 작업공간을, 둘째는 mallory가 시도할(그리고 실패할) 작업공간을
    // mint한다 — 서로 다른 고정 바이트라 두 id가 겹치지 않는다.
    ...
    const database = await openTestControlDatabase()
    const store = await openWorkspaceStore(database, { randomBytes: sequencedRandomBytes })
    const { workspaceId: aliceWorkspaceId } = await insertWorkspace(database, store, 'alice', { logs: ['log-a'] })

    const predictedId = store.mintWorkspaceId()
    expect(predictedId).not.toBe(aliceWorkspaceId)

    await expect(
      database.withTransaction(() => {
        store.insertMintedWorkspace(predictedId, 'mallory', { logs: ['log-b'], supersedes: aliceWorkspaceId })
      }),
...
): Promise<{ workspaceId: string; openedAt: number }> {
  const { workspaceId } = await insertWorkspace(database, store, 'alice', { logs: ['log-a'] })
...
describe('WorkspaceStore 전이 — 하트비트·종료·폐기', () => {
  it('하트비트가 lastHeartbeatAt을 앞으로 옮기고 state는 active 그대로다', async () => {
    ...
    const { workspaceId, openedAt } = await openAt(database, store, gracePeriodMs)
  ...
  it('revokeWorkspace가 멱등이고, closed_*인 작업공간에 대한 폐기는 실패한다', async () => {
    ...
    const other = await openAt(database, store, gracePeriodMs)
...
describe('WorkspaceStore.listWorkspaces', () => {
  it('필터 없이 나열하면 openedAt 오름차순이고, 다른 주체의 작업공간은 실리지 않는다', async () => {
    ...
    const [second, third, first] = await openSequenced(database, store, 'alice', [2_000, 3_000, 1_000])
    await openSequenced(database, store, 'mallory', [1_500])
  ...
  it('§4.7 관찰 조건: 하트비트 뒤 방치된 작업공간이 state=abandoned 필터에 잡히고, endedAt이 lastHeartbeatAt + gracePeriod이며 logs가 개시 시 스코프 그대로다', async () => {
    ...
    const store = await openWorkspaceStore(database)
    const gracePeriodMs = 1_000
    const [workspaceId] = await openSequenced(database, store, 'alice', [0])
  ...
  it('필터 → 정렬 → limit 순서: 유기 1건 + 그보다 나중에 열린 active 여러 건에서 state=abandoned + limit=1이 그 유기 1건을 돌려주고 hasMore가 false다', async () => {
    ...
    const [abandonedId] = await openSequenced(database, store, 'alice', [0])
    await openSequenced(database, store, 'alice', [gracePeriodMs + 1, gracePeriodMs + 2])
  ...
  it('커서로 다음 페이지를 이어 받으면 앞 페이지 항목이 반복되지 않고 전량이 정확히 한 번씩 나온다', async () => {
    ...
    const store = await openWorkspaceStore(database)
    const gracePeriodMs = 60_000
    const ids = await openSequenced(database, store, 'alice', [0, 1_000, 2_000, 3_000, 4_000])
  ...
  it('mori-nest #109: 승계자가 둘 이상인 작업공간을 포함한 목록 조회에서 supersededBy가 getWorkspace와 같은 값(workspaceId 사전순 최소)이고, 승계되지 않은 행에는 그 필드가 없다', async () => {
    ...
    const [target] = await openSequenced(database, store, 'alice', [0])
    const [untouched] = await openSequenced(database, store, 'alice', [1_000])
    await insertWorkspace(database, store, 'alice', { logs: ['log-a'], supersedes: target! })
    await insertWorkspace(database, store, 'alice', { logs: ['log-a'], supersedes: target! })
  ...
  it('state가 다섯 이름 밖이면 실패하고, 해석 불가 커서도 실패한다 (둘 다 «전부 반환»으로 떨어지지 않는다)', async () => {
    ...
    const store = await openWorkspaceStore(database)
    const gracePeriodMs = 60_000
    await openSequenced(database, store, 'alice', [0])
...
async function openReplicaAt(
  ...
  const { workspaceId } = await insertWorkspace(database, store, subject, {
    logs: ['log-a'],
...
describe('WorkspaceStore.findForkAdvisory (§4.10 포크 판정, advisory 조각 1/2)', () => {
  it('겹치면 advisory가 나온다 — 같은 replicaId의 두 active, 그리고 abandoned(파생) ↔ active', async () => {
    ...
    const database1 = await openTestControlDatabase()
    const store1 = await openWorkspaceStore(database1)
    vi.useFakeTimers()
    const a1 = await openReplicaAt(database1, store1, 'alice', 'r1', 0)
    const b1 = await openReplicaAt(database1, store1, 'alice', 'r1', 1_000)
    vi.useRealTimers()
    ...
    const database2 = await openTestControlDatabase()
    const store2 = await openWorkspaceStore(database2)
    const gracePeriodMs2 = 1_000
    vi.useFakeTimers()
    const a2 = await openReplicaAt(database2, store2, 'alice', 'r1', 0)
    const b2 = await openReplicaAt(database2, store2, 'alice', 'r1', 500)
    vi.useRealTimers()
  ...
  it('안 겹치면 undefined다 — close 뒤에 열린 경우와, endedAt과 openedAt이 정확히 맞닿는 경계 접촉', async () => {
    ...
    const b1 = await openReplicaAt(database1, store1, 'alice', 'r1', 200)
    vi.useRealTimers()
    ...
    const c = await openReplicaAt(database2, store2, 'alice', 'r1', 100)
    vi.useRealTimers()
  ...
  it('제외 규칙 — 자기 자신뿐인 경우, 다른 주체의 같은 replicaId, 상대의 replicaId 미신고는 모두 undefined다', async () => {
    ...
    const gracePeriodMs = 60_000
    vi.useFakeTimers()
    const self = await openReplicaAt(database, store, 'alice', 'r1', 0)
    vi.useRealTimers()
    ...
    vi.useFakeTimers()
    await openReplicaAt(database, store, 'mallory', 'r1', 500)
    vi.useRealTimers()
    expect(await store.findForkAdvisory('alice', self, { gracePeriodMs, now: new Date(1_000) })).toBeUndefined()

    // 같은 주체·겹치는 시간이라도 상대가 replicaId를 신고하지 않았으면 비교되지 않는다.
    vi.useFakeTimers()
    await openReplicaAt(database, store, 'alice', undefined, 600)
    vi.useRealTimers()
    expect(await store.findForkAdvisory('alice', self, { gracePeriodMs, now: new Date(1_000) })).toBeUndefined()
    ...
    vi.useFakeTimers()
    const silent = await openReplicaAt(database, store, 'alice', undefined, 700)
    vi.useRealTimers()
    ...
    vi.useFakeTimers()
    await openReplicaAt(database, store, 'alice', 'r2', 800)
    vi.useRealTimers()
    expect(await store.findForkAdvisory('alice', self, { gracePeriodMs, now: new Date(1_000) })).toBeUndefined()
  })
})
