...
이 문서는 아직 아래 절 둘만 담는다. 다른 절(코딩 스타일, 커밋 메시지, 이슈 작성 규칙,
...
**닫기 키워드(`Closes #N` 형태)는 백틱(코드 스팬) 밖, 본문 마지막 줄에 단독 텍스트로 쓴다.**
코드 스팬이나 코드 블록 안에 넣으면 GitHub이 이를 파싱하지 않아 PR이 머지돼도 이슈가
닫히지 않는다.

이 리포의 이슈는 라벨로 폴링된다. 닫히지 않은 이슈는 범위가 이미 PR로 전달됐는데도
상태 라벨(`agent:review` 등)에 그대로 멈춰, 어느 폴링 패스도 다시 집지 않는 고아가 된다.
(선례: 이슈 #82는 PR #86이 범위를 전부 전달하고 머지됐는데도, PR 본문의 닫기 키워드가
백틱 안에 있었던 탓에 연결이 성립하지 않아 26시간 넘게 `agent:review`에 고아로 남았고,
결국 owner가 직접 닫아 정리했다.)

> 주의 — 위 규칙 설명에서 `` `Closes #N` ``을 백틱으로 감싼 것은 이 문서 안에서는
> 무해하다(이 문서는 PR 본문이 아니다). 하지만 그 표기를 그대로 복사해 **PR 본문에** 붙이면
> 안 된다 — #82 사고가 정확히 이 경로로 일어났다.
