 ...
 *
 * 예외는 `token.js`·`request.js`·`event.js`다 — 전송 평면의 게이트(0003 §3.3 · 0002 §1.2 ·
 * §1.3·§2.1)이므로 평면에 속하지만, 서버도 저장소도 모르는 순수 함수라 여기서 함께 나간다
 * (mori-nest #11 · #14 · #17). `request.js`는 라우트 **표**를 갖지만 라우트 **핸들러**는 갖지
 * 않는다 — 판정 결과를 반환할 뿐 HTTP 응답을 쓰지 않는다. `event.js`는 append **본문**의
 * 게이트이고, 그 둘을 잇는 것(요청 게이트 → 본문 게이트)은 서버 조각의 몫이다.
 */
