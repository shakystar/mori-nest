 ...
 * ## 오늘 이 평면에 라우트는 0건이다
 *
 * `0003 §8-2`·`§8-3`이 mori-nest #68 사람 결정으로 닫히면서 `logId` mint와 (주체, 로그) 관계가
 * `./store.js`로 섰고(mori-nest #68 조각 3/5 · #72), `Idempotency-Key` 계층(조각 4/5,
 * `0003 §1.4`)이 `./idempotency.js`로 섰다 — 두 라우트(`POST /v1/logs`·`POST /v1/workspaces`)가
 * 공유해서 탈 재사용 가능한 계층이고, 자원 스토어와 독립이라 먼저 설 수 있었다. 런처
 * 자격증명의 발급·조회 판정·즉시 폐기도 `./credential.js`로 섰다(조각 5/5 · #74). 그래도
 * 제어 평면 6 라우트와 서버는 여전히 여기 없다 — 라우트 배선 자체가 다음 사이클 조각이기
 * 때문이다(#74 비범위). 발급 HTTP 라우트도 이 조각의 비범위다(`§1.1` — "오늘은 운영자가
 * 손으로 발급하는 것으로 족하다"). 발급에 필요한 나머지 설정(서명 키의 `keyId`, 토큰
 * 수명 등)도 그것을 쓰는 라우트와 함께 온다 — 쓰는 코드가 없는 필드를 스키마에 미리
 * 만들지 않는다.
 *
 * ## 한 앱 두 포트로도, 두 앱으로도
 *
