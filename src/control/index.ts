 ...
 *
 * 오늘 이 연결로 옮겨온 것은 `./idempotency.js` · `./credential.js`(조각 1/4)와 `./store.js`
 * (조각 2/4, mori-nest #131) 셋이다. `./workspace-store.js`는 아직 자기 경로로 연다 —
 * **조각 3/4**에서 이관하는 의도된 과도기다. 라우트를 한 트랜잭션으로 묶어 실패 창을 실제로
 * 닫는 것은 **조각 4/4**다.
 *
 * ## 한 앱 두 포트로도, 두 앱으로도
 *
