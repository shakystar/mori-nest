 ...
 *
 * `src/control/store.ts`는 **조각 2/4**(mori-nest #131)에서, `src/control/workspace-store.ts`는
 * **조각 3/4**(mori-nest #132)에서 이 연결로 왔다. 라우트를 한 트랜잭션으로 묶어 실패 창을
 * 실제로 닫는 것만 **조각 4/4**로 남았다.
 *
