 ...
 *   처리하므로 여기서 걸리지 않는다.
 * - 빈 문자열 — 원문 조각이 비면 `{"id":…,"payload":,"cursor":…}`가 되어 **JSON으로 파싱되지
 *   않는 프레임**이 나간다. 줄 구조는 멀쩡하므로 클라이언트는 프레임을 받아 놓고 본문에서
 *   깨진다. `event.ts`는 빈 범위를 `500`으로 막으므로(*"스캐너와 파서가 갈라졌다"*) 게이트를
 *   지나온 값에는 이 경우가 없지만, **이 함수의 안전성이 부르는 쪽의 규율에 의존하지 않게**
 *   여기서 함께 막는다 — 조건이 빈 문자열에서 통째로 꺼지는 것이 이 판정의 실패 모드다.
 */
function isRepresentablePayload(payload: string): boolean {
  return payload !== '' && !payload.includes('\r')
}
